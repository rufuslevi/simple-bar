...
export const stopSliding = (container, sliderSelector) => {
    if (!container || !container.querySelector(sliderSelector)) return;
    container.querySelector(sliderSelector).removeAttribute("style");
};

export const cleanupOutput = (output) =>
    output?.trim().replace(/(\r\n|\n|\r)/gm, "");
...
export const switchSpace = async (currentIndex, desiredIndex) => {
    ...
    for (let i = 0; i < repeats; i++) {
        await Uebersicht.run(
            `osascript -e 'tell app "System Events" to key code ${left ? "123" : "124"
            } using control down'`
        );
    }
};
